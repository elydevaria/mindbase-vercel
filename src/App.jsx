        ...
        <div ref={chatRef} style={{ flex:1, overflowY:"auto", padding:"22px 24px 0", display:"flex", flexDirection:"column", gap:14 }}>
          {messages.length===0 && (
            <div style={{ background:"#fff", border:"1px solid #e2ddd5", borderRadius:14, padding:26 }}>
              <div style={{ fontFamily:"Georgia,serif", fontSize:25, color:"#1c1917", marginBottom:8 }}>Bonjour, <em style={{ color:"#2d5a3d" }}>praticien(ne)</em> 👋</div>
              <div style={{ fontSize:13, color:"#6b6560", lineHeight:1.75, marginBottom:18, maxWidth:520 }}>Posez une question sur n'importe quel sujet clinique — je structure ma réponse en sections : livres, vidéos, Instagram, Facebook, Reddit, recherches, recommandations HAS. Sauvegardez ce qui vous est utile dans votre bibliothèque.</div>
          ...
          {messages.map((msg,i)=>(
            <div key={i} style={{ display:"flex", gap:10, flexDirection:msg.type==="user"?"row-reverse":"row", alignItems:"flex-start" }}>
              ...
              <div style={{ maxWidth:"78%" }}>
                ...
                {msg.type==="agent"&&<div style={{ display:"flex", justifyContent:"flex-end", marginTop:5 }}><button onClick={()=>setSaveTarget(msg)} style={{ display:"flex", alignItems:"center", gap:5, padding:"4px 10px", border:"1px solid #e2ddd5", borderRadius:20, background:"#fff", cursor:"pointer", fontSize:11, color:"#6b6560", fontFamily:"system-ui,sans-serif" }} onMouseEnter={e=>{e.currentTarget.style.background="#eaf3ee";e.currentTarget.style.color="#2d5a3d";e.currentTarget.style.borderColor="#a8cdb5";}} onMouseLeave={e=>{e.currentTarget.style.background="#fff";e.currentTarget.style.color="#6b6560";e.currentTarget.style.borderColor="#e2ddd5";}}>🔖 Sauvegarder</button></div>}
              </div>
            </div>
          ))}
